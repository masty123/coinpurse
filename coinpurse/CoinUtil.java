...
public class CoinUtil {
	...
	public static List<Valuable> filterByCurrency(final List<Valuable> coinlist, String currency) {
		...
		return Collections.unmodifiableList(temp); // return a list of coin
	...
	public static void sumByCurrency(List<Valuable> coins) {
		...
		for (Valuable money : coins) {
			map.put(money.getCurrency(), map.getOrDefault(money.getCurrency(), 0.0) + money.getValue());
			// if (map.containsKey(money.getCurrency())) {
			// map.put(money.getCurrency(), map.get(money.getCurrency() +
			// money.getValue()));
			// } else
